...
## 🛠️ Action Step 1: Simulating the Fraud API

Since we don't have a real external fraud service, we will build a realistic Mock API using `designer.mocky.io`. This will allow us to test our Databricks Secret Scope authentication.

1. Open your local terminal, navigate to `apps/data-generator/`, and run `python generate_fraud.py --rows 500`.
2. Open the newly generated `data/raw/fraud/fraud_blacklist.json` file and copy its contents.
3. Open your browser and navigate to [designer.mocky.io](https://designer.mocky.io/).
4. Paste the JSON contents into the **HTTP Response Body**.
5. Click **Advanced Configuration** and add an HTTP Header requirement:
   - Header Name: `Authorization`
   - Header Value: `Bearer my-super-secret-token`
6. Click **Generate my HTTP Response** and copy the resulting URL.

*(Note: In a real Databricks environment, you must ensure `my-super-secret-token` is saved in your Azure Key Vault and linked to the `mortgage-secrets` Databricks Secret Scope!)*

---

## 🛠️ Action Step 2: Ingesting the Fraud Blacklist
...
4. Hit your custom Mocky API and save the raw JSON payload to the ADLS Gen2 landing zone (`abfss://bronze@.../landing/fraud_blacklist/blacklist_today.json`).
...
## 🛠️ Action Step 3: Local Unit Testing for HTTP APIs
...
def test_fetch_fraud_blacklist_success():
    ...
    mock_payload = [
        ...
        {"fraud_id": "456", "entity_type": "Email", "malicious_value": "scammer@bad.com"}
    ]
